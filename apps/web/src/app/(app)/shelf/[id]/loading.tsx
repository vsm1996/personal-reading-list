...
export default function ShelfDetailLoading() {
  return (
    <div className="mx-auto max-w-[var(--container-library)] animate-pulse px-6 py-8">
      {/* Back link skeleton */}
      <div className="mb-6 h-4 w-16 rounded bg-[var(--color-bg-tertiary)]" />
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <div className="flex gap-1">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-7 w-24 rounded-md bg-[var(--color-bg-tertiary)]" />
          ))}
        </div>
      </div>
      ...
      <ul className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-6">
        {Array.from({ length: 12 }).map((_, i) => (
          <li key={i}>
            <div className="aspect-[2/3] w-full rounded bg-[var(--color-bg-tertiary)]" />
